...
class Mdct {
    ...
    void init(int n) {
        ...
        int CO = CE + 1;
        for (i = 0; i < n / 4; ++i) {
            this.trig[AE + i * 2] = (float)Math.cos(3.141592653589793 / (double)n * (double)(4 * i));
            this.trig[AO + i * 2] = (float)(- Math.sin(3.141592653589793 / (double)n * (double)(4 * i)));
            this.trig[BE + i * 2] = (float)Math.cos(3.141592653589793 / (double)(2 * n) * (double)(2 * i + 1));
            this.trig[BO + i * 2] = (float)Math.sin(3.141592653589793 / (double)(2 * n) * (double)(2 * i + 1));
        }
        for (i = 0; i < n / 8; ++i) {
            this.trig[CE + i * 2] = (float)Math.cos(3.141592653589793 / (double)n * (double)(4 * i + 2));
            this.trig[CO + i * 2] = (float)(- Math.sin(3.141592653589793 / (double)n * (double)(4 * i + 2)));
        }
    ...
    synchronized void backward(float[] in, float[] out) {
        ...
        int A = n2;
        for (i = 0; i < n8; ++i) {
            x[xO++] = (- in[inO + 2]) * this.trig[A + 1] - in[inO] * this.trig[A -= 2];
            x[xO++] = in[inO] * this.trig[A + 1] - in[inO + 2] * this.trig[A];
            inO += 4;
        }
        inO = n2 - 4;
        for (i = 0; i < n8; ++i) {
            x[xO++] = in[inO] * this.trig[A + 1] + in[inO + 2] * this.trig[A -= 2];
    ...
    private float[] mdct_kernel(float[] x, float[] w, int n, int n2, int n4, int n8) {
        ...
        for (i = 0; i < n4; ++i) {
            ...
            w[w2 + i] = x[xA++] + x[xB++];
            float x1 = x[xA] - x[xB];
            w[i++] = x0 * this.trig[A] + x1 * this.trig[(A -= 4) + 1];
