...
extends FuncFloor {
    ...
    void pack(Object i, Buffer opb) {
        ...
        k = 0;
        for (j = 0; j < info.partitions; ++j) {
            while (k < (count += info.class_dim[info.partitionclass[j]])) {
    ...
    Object look(DspState vd, InfoMode mi, Object i) {
        ...
        for (j2 = 0; j2 < _n - 1; ++j2) {
            for (int k = j2; k < _n; ++k) {
                if (info.postlist[sortpointer[j2]] <= info.postlist[sortpointer[k]]) continue;
                int foo = sortpointer[k];
                sortpointer[k] = sortpointer[j2];
                sortpointer[j2] = foo;
            }
        }
        for (j2 = 0; j2 < _n; ++j2) {
        ...
        for (j2 = 0; j2 < _n - 2; ++j2) {
            ...
            look.loneighbor[j2] = lo;
            look.hineighbor[j2] = hi;
        }
        return look;
    }
        ...
        if (vb.opb.read(1) == 1) {
            ...
            for (i = 2; i < look.posts; ++i) {
                ...
                if (val != 0) {
                    ...
                    arrn2[n2] = arrn2[n2] & 32767;
                    continue;
                }
                fit_value[i] = predicted | 32768;
            }
            return fit_value;
        }
        return null;
    ...
    private static void render_line(int x0, int x1, int y0, int y1, float[] d) {
        ...
        float[] arrf = d;
        int n = x;
        arrf[n] = arrf[n] * FLOOR_fromdB_LOOKUP[y];
        while (++x < x1) {
            if ((err += ady) >= adx) {
                err -= adx;
                y += sy;
            ...
            float[] arrf2 = d;
            int n2 = x;
            arrf2[n2] = arrf2[n2] * FLOOR_fromdB_LOOKUP[y];
        }
    }
