...
public class VorbisFile {
    ...
    private int get_prev_page(Page page) throws JOrbisException {
        ...
        block0 : while (offst == -1) {
            ...
            while (this.offset < begin + 8500) {
                ret = this.get_next_page(page, begin + 8500 - this.offset);
                if (ret == -128) {
                    return -128;
                }
                if (ret < 0) {
        ...
        ret = this.get_next_page(page, 8500);
        if (ret < 0) {
            return -129;
        }
        return offst;
    ...
    int bisect_forward_serialno(long begin, long searched, long end, int currentno, int m) {
        ...
        while (searched < endsearched) {
            long bisect = endsearched - searched < 8500 ? searched : (searched + endsearched) / 2;
    ...
    int fetch_headers(Info vi, Comment vc, int[] serialno, Page og_ptr) {
        ...
        if (og_ptr == null) {
            int ret = this.get_next_page(og, 8500);
            if (ret == -128) {
                return -128;
            }
            if (ret < 0) {
        ...
        while (i < 3) {
            ...
            if (i >= 3 || this.get_next_page(og_ptr, 1) >= 0) continue;
            vi.clear();
            vc.clear();
            this.os.clear();
            return -1;
    ...
    int open_seekable() throws JOrbisException {
        ...
        this.seekable = true;
        VorbisFile.fseek(this.datasource, 0, 2);
        ...
        } else if (this.bisect_forward_serialno(0, end, end + 1, serialno, 0) < 0) {
            this.clear();
            return -128;
        }
    ...
    int open_callbacks(InputStream is, byte[] initial, int ibytes) throws JOrbisException {
        ...
        int ret = is instanceof SeekableInputStream ? this.open_seekable() : this.open_nonseekable();
        if (ret != 0) {
    ...
    public int bitrate(int i) {
        ...
        if (this.seekable) {
            return (int)Math.rint((float)((this.offsets[i + 1] - this.dataoffsets[i]) * 8) / this.time_total(i));
        }
    ...
    public int pcm_seek(long pos) {
        ...
        Page og = new Page();
        while (begin < end) {
            long bisect = end - begin < 8500 ? begin : (end + begin) / 2;
            this.seek_helper(bisect);
